...
{ 
    ...
    this->progressLabel->setText(tr("NOTE!!: Existing Patch data will be replaced"));
    ...
    {
        QString text = "CH:";
        text.append(QString::number(p-1, 10).toUpper());
        if (p==1) {text="Knob Panel"; };
        p++;        
...
{ 
    ...
    QString v;
    QString replyMsg;
    if (patchRange>=128) {patchRange=patchRange-128; addrMSB = "11"; };          // next address range when > 10 7F.
    ...
    if (steps<((finish-start)+1) )
    {
        ...
        patchNumber.prepend(tr( "Channel" ));
        patchNumber.append("-");
...
void bulkEditDialog::prepareData()
{
    ...
    if( /*sysxMsg.size()/2 == fullPatchSize &&*/ partialStart>0 && partialLength>0)
    {
        ...
        this->partialData = "F04100000060126000" + this->hex1 + this->hex3 + temp + "00F7";
    } else {
        QApplication::beep();
...
void bulkEditDialog::DialogClose()
{
    bank = 100;
    SysxIO *sysxIO = SysxIO::Instance();
    QObject::disconnect(sysxIO, SIGNAL(sysxReply(QString)), this, SLOT(sendSequence(QString)));
