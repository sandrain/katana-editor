...
void customPanelButton::customPanelLabel(QString text){
    ...
    textLabel = new QLabel(this->parentWidget());
    textLabel->setObjectName("panelButton");
    QFont Mfont( "Arial", 8*fratio, QFont::Bold);
    textLabel->setFont(Mfont);
    textLabel->setText(text);
    textLabel->setAlignment(Qt::AlignCenter);
    textLabel->setGeometry(buttonPos.x()-((buttonSize.width()-3)*ratio), (buttonPos.y()+((buttonSize.height()-3)*ratio)), textLabel->width()*ratio, (textLabel->height()-10)*ratio);
}
