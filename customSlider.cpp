    ...
    : QWidget(parent)
{
    ...
    this->slideSize = QPixmap(this->slideImagePath).size()*ratio;
    this->sliderButtonSize = QPixmap(this->sliderButtonImagePath).size()*ratio;
...
void customSlider::paintEvent(QPaintEvent *)
{
    ...
    buffer = buffer.scaled(slideSize.width(), slideSize.height(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
void customSlider::setOffset(double _newValue)
{
    ...
    double range = (slideSize.height()  - sliderButtonSize.height())*50/70;
